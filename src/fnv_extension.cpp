...
inline void FnvScalarFun(DataChunk &args, ExpressionState &state, Vector &result) {
...
static void LoadInternal(DatabaseInstance &instance) {
    ...
    auto fnv_scalar_function = ScalarFunction("fnv", {LogicalType::VARCHAR, LogicalType::INTEGER}, LogicalType::INTEGER, FnvScalarFun);
    ExtensionUtil::RegisterFunction(instance, fnv_scalar_function);
}

void FnvExtension::Load(DuckDB &db) {
	LoadInternal(*db.instance);
}
std::string FnvExtension::Name() {
	return "fnv";
}
